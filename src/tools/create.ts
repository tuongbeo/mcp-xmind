...
export function registerCreateTool(server: McpServer, env: Env): void {
  server.registerTool('create_xmind', {
    ...
    await kv.putFile(fileKey, built);
    const now = new Date().toISOString();
    await kv.index(fileKey, { fileName: safeName, fileKey, fileSize: built.byteLength, sheetCount: docSheets.length, nodeCount, createdAt: now, updatedAt: now });
